...
import type { Json, QuoteStatus, WorkType } from "@/lib/supabase/types";
...
/* ---------- select question options (survey_template_items.options) ---------- */

/* The options of a select question when they are stored as an array of strings. */
export function optionStrings(options: Json | null | undefined): string[] {
  if (!Array.isArray(options)) return [];
  return options.filter((o): o is string => typeof o === "string" && o.trim().length > 0);
}

/* Stored array to the text of the editor, one option per line. */
export function optionsText(options: Json | null | undefined): string {
  return optionStrings(options).join("\n");
}

/* Editor text to the stored array: trimmed lines, blanks and repeats dropped, null when empty. */
export function optionsFrom(text: string): string[] | null {
