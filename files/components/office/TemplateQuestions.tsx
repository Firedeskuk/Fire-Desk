...
import { assertOk, errorMessage, must, optionStrings, optionsFrom, optionsText } from "./helpers";
...
type Draft = {
  ...
  required: boolean;
  fail_values: string;
  /* one option per line, select questions only */
  options: string;
};

const BLANK: Draft = { question: "", answer_type: "yes_no", required: true, fail_values: "no", options: "" };
...
/* What Add and Save write for the options column. Only select questions keep options. */
function optionsPayload(d: Draft): string[] | null {
  return d.answer_type === "select" ? optionsFrom(d.options) : null;
}
...
/* The current options of a select question as small pills. */
function OptionPills({ options }: PillsProps) {
  if (options.length === 0) {
    return <span className="muted small">No options yet</span>;
  }
  return (
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
      {options.map((o) => (
        <span key={o} className="pill small" style={{ minHeight: 24, padding: "2px 10px" }}>
          {o}
        </span>
      ))}
    </div>
...
function DraftFields({ draft, onChange, disabled }: FieldsProps) {
  ...
  return (
    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
      ...
      {draft.answer_type === "select" ? (
        <div style={{ flex: "1 1 100%" }}>
          <Field label="Options, one per line" hint="What the inspector picks from. Fail values must match an option.">
            <textarea
              className="textarea"
              value={draft.options}
...
type InlineOptionsProps = {
  ...
  disabled: boolean;
  onSave: (item: SurveyTemplateItem, options: string[] | null) => void;
};

/*
  Inline options editor under a select question. Uncontrolled, keyed by the
  saved text so it remounts with the new default after a save. Saved on
  blur when the text changed.
*/
function InlineOptions({ item, disabled, onSave }: InlineOptionsProps) {
  const saved = optionsText(item.options);
  return (
    <div style={{ marginTop: 8 }}>
      <OptionPills options={optionStrings(item.options)} />
      <label className="field" style={{ marginTop: 8, marginBottom: 0 }}>
        ...
        <textarea
          ...
          placeholder={"Good\nWorn\nMissing"}
          onBlur={(e) => {
            const next = optionsFrom(e.currentTarget.value);
            const current = optionsFrom(saved);
...
/*
  ...
  A select question shows its options as pills and an inline editor that
  saves options as a JSON array of strings on blur.
*/
export default function TemplateQuestions({ template }: Props) {
      ...
      assertOk(
        await getSupabase()
          ...
          .update({
            ...
            options: optionsPayload(editing),
          })
          .eq("id", id),
      );
      setEditingId(null);
  ...
  function saveOptions(item: SurveyTemplateItem, options: string[] | null) {
    void run(async () => {
      assertOk(await getSupabase().from("survey_template_items").update({ options }).eq("id", item.id));
    });
  }
    ...
    <section className="card" style={{ marginTop: 0 }} aria-label="Questions">
      ...
      {items.map((item, index) =>
        editingId === item.id ? (
          ...
          <div className="row" key={item.id} style={{ flexWrap: "wrap", alignItems: "flex-start" }}>
            <div className="muted" style={{ minWidth: 28, paddingTop: 2 }}>
              {index + 1}
            </div>
            <div className="row-main">
              ...
              {item.answer_type === "select" ? (
                <InlineOptions item={item} disabled={busy} onSave={saveOptions} />
              ) : null}
            </div>
